...
public:
    int maxCandies(vector<int> &status, vector<int> &candies, vector<vector<int>> &keys, vector<vector<int>> &containedBoxes, vector<int> &initialBoxes)
    {
        ...
        while (!q.empty())
        {
            ...
            {
                key.insert(it);
                if (vis[it] == 0 && (boxes.find(it) != boxes.end()))
                {
                    vis[it] = 1;
                    q.push(it);
                }
            ...
            {
                boxes.insert(it);
                if (status[it] == 1 || (key.find(it) != key.end()))
                {
                    q.push(it);
                    vis[it] = 1;
                }
...
/*
Time Complexity: O(n)
Space Complexity: O(n), where n is the number of boxes. We use a queue and sets to keep track of boxes, keys, and visited status.
*/
...
int main()
{
    ...
    cout << "Maximum candies collected: " << result << endl; // Output: 16
